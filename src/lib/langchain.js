...
// RAG prompt template
const ragPrompt = PromptTemplate.fromTemplate(`You are a helpful AI assistant that answers questions based strictly on the provided context. 
...
export async function generateSummary() {
  try {
    ...
    const docs = await searchSimilarDocuments('summary main topics content', 10);
    
    if (docs.length === 0) {
      return "No sources have been uploaded yet. Please upload some documents first.";
    }

    const summaryPrompt = PromptTemplate.fromTemplate(`You are a helpful AI assistant. Please provide a comprehensive summary of the following sources.
